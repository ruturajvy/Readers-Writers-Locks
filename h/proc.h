...
struct	pentry	{
	...
	int	pwaitret;
	int	lock_return;		/* Return value 'OK' or 'DELETED'*/
	unsigned long	lock_time;		/* The milisecond of waiting time for lock*/
	int	lock_list[NLOCK];	/* Bit mask of the locks held with their type*/
	int	lock_q;			/* Lock descriptor of  the queue in which the process is waiting */
	int	acquired[NLOCK];	/* Bit mask of the processes acquired */
